...
public class SomaticGenotypingEngine extends AssemblyBasedCallerGenotypingEngine {
    ...
    public CalledHaplotypes callMutations(
        ...
        for( final int loc : startPosKeySet ) {
            ...
            final PerAlleleCollection<Double> tumorLog10Odds = somaticLog10Odds(log10TumorMatrix, Strand.BOTH);
            final Optional<PerAlleleCollection<Double>> normalLog10Odds = getForNormal(() -> diploidAltLog10Odds(log10NormalMatrix.get()));
            final Optional<PerAlleleCollection<Double>> normalArtifactLods = getForNormal(() -> somaticLog10Odds(log10NormalMatrix.get(), Strand.BOTH));
            ...
            addStrandBiasAnnotations(subsettedLog10TumorMatrix, alleleWithHighestTumorLOD, callVcb);
            ...
            final ReadLikelihoods<Allele> log10LikelihoodsForAnotations = prepareReadAlleleLikelihoodsForAnnotation(log10ReadLikelihoods, perSampleFilteredReadList,
    ...
    private PerAlleleCollection<Double> somaticLog10Odds(final LikelihoodMatrix<Allele> log10Matrix, final Strand strand) {
        final double log10EvidenceWithAllAlleles = !hasReadsFromStrand(log10Matrix, strand) ? 0 :
                SomaticLikelihoodsEngine.log10Evidence(getAsRealMatrix(log10Matrix, strand));
        ...
        IntStream.range(0, log10Matrix.numberOfAlleles()).filter(a -> a != refIndex).forEach( a -> {
            ...
            final double log10EvidenceWithoutThisAllele = !hasReadsFromStrand(log10MatrixWithoutThisAllele, strand) ? 0 :
                    SomaticLikelihoodsEngine.log10Evidence(getAsRealMatrix(log10MatrixWithoutThisAllele, strand));
    ...
    private VariantContext addGenotypes(final LikelihoodMatrix<Allele> tumorLog10Matrix,
        ...
        final double[] tumorAlleleCounts = SomaticLikelihoodsEngine.getEffectiveCounts(getAsRealMatrix(tumorLog10Matrix, Strand.BOTH));
        ...
        if (hasNormal) {
            final double[] normalAlleleCounts = SomaticLikelihoodsEngine.getEffectiveCounts(getAsRealMatrix(normalLog10Matrix.get(), Strand.BOTH));
    ...
    private void addStrandBiasAnnotations(final LikelihoodMatrix<Allele> log10TumorLikelihoods, final Allele alleleWithHighestTumorLOD, final VariantContextBuilder callVcb) {
        final PerAlleleCollection<Double> tumorForwardLogOdds = somaticLog10Odds(log10TumorLikelihoods, Strand.FORWARD);
        final PerAlleleCollection<Double> tumorReverseLogOdds = somaticLog10Odds(log10TumorLikelihoods, Strand.REVERSE);

        final List<GATKRead> tumorReads = log10TumorLikelihoods.reads();
    ...
    public static RealMatrix getAsRealMatrix(final LikelihoodMatrix<Allele> matrix, final Strand strand) {
        final int[] strandReadIndices = IntStream.range(0, matrix.numberOfReads())
                .filter(n -> Strand.readComesFromStrand(matrix.getRead(n), strand)).toArray();
        final RealMatrix result = new Array2DRowRealMatrix(matrix.numberOfAlleles(), strandReadIndices.length);
        result.walkInOptimizedOrder(new DefaultRealMatrixChangingVisitor() {
            @Override
            public double visit(int row, int column, double value) {
                return matrix.get(row, strandReadIndices[column]);
            }
        });
        return result;
    }
    ...
    private boolean hasReadsFromStrand(final LikelihoodMatrix<Allele> matrix, final Strand strand) {
        return matrix.reads().stream().anyMatch(read -> Strand.readComesFromStrand(read, strand));
    }
